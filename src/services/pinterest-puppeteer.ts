...
export default class PinterestPuppeteer {
  ...
  async createIncognitoPage(): Promise<Page> {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        this.errorHandler.logError(
          `Login attempt ${attempt}/${retries} failed`,
          error as Error
        );
        if (attempt < retries) {
  ...
  async scrapeURL(
    ...
    // Create fresh incognito page for each URL
    const scrapePage = await this.createIncognitoPage();

    // Close old scrape page if exists (cleanup)
    if (scrapePage && scrapePage !== scrapePage) {
      try {
        const context = scrapePage.browserContext();
        await context.close();
      } catch (e) {
    ...
    try {
      ...
      console.log(
        chalk.gray(`  Starting auto-scroll (stops when no new content)...`)
      );

      // Extract initial pins
      await this.extractPins(scrapePage, pinDataList, processedIds, url);
      console.log(chalk.gray(`    Initial load: ${pinDataList.length} pins`));

      // OPTIMIZED: Scroll much faster
      while (
        scrollsPerformed < maxScrollAttempts &&
        noNewContentCount < noNewContentLimit
      ) {
        ...
        for (let step = 0; step < scrollSteps; step++) {
          ...
          await scrapePage.evaluate(() => window.scrollBy(0, 200));
        ...
        await scrapePage.evaluate(() => window.scrollBy(0, 500));
        ...
        if (newPinsCount > 0) {
          ...
          console.log(
            chalk.gray(
              `    Scroll ${scrollsPerformed}: +${newPinsCount} new pins (Total: ${pinDataList.length})`
            )
          );
        } else {
          noNewContentCount++;
          console.log(
            chalk.yellow(
              `    Scroll ${scrollsPerformed}: No new content (${noNewContentCount}/${noNewContentLimit})`
            )
          );
        }
      }
      ...
      if (scrollsPerformed >= maxScrollAttempts) {
        console.log(
          chalk.yellow(`  ⚠ Reached max scroll limit (${maxScrollAttempts})`)
        );
      } else {
      ...
      // Always cleanup the incognito context
      try {
        const context = scrapePage.browserContext();
        await context.close();
      } catch (e) {
  ...
  private async extractPins(
    ...
    try {
      ...
      const imagePattern =
        /https:\/\/i\.pinimg\.com\/[^"'\s]+\.(jpg|png|webp)/gi;
      let match;
  ...
  private findExactPinLink(
    ...
    const pinLinksBeforeImage = [
      ...beforeContext.matchAll(/href="(\/pin\/\d+[^"]*?)"/g),
    ];
  ...
  private getOriginalUrl(url: string): string {
    return url
      .replace(
        /\/(originals|236x|474x|564x|736x|170x|videos\/thumbnails)\//,
        "/originals/"
      )
      .replace(/_RS\//, "/")
